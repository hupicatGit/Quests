...
export const GameLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    const { visuals, ui, toggleAppreciateMode } = useGameStore();
    ...
    return (
        <div className={`w-[100vw] h-[100vh] bg-black overflow-hidden relative selection:bg-indigo-500/30 ${screenShake ? 'animate-shake' : ''}`}>
            ...
            {/* 2. 靠左/居中的大比例角色立绘 */}
            <div
                className={`absolute left-[5%] md:left-[10%] lg:left-[15%] bottom-0 h-[80vh] md:h-[90vh] z-10 transition-all duration-[1500ms] ease-out pointer-events-none drop-shadow-[0_0_50px_rgba(0,0,0,0.8)]
