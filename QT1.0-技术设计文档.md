...
| **叙事区域** | 右侧面板 (宽约 `35vw`，最大 `600px`) | 对话/叙述文本。背景使用极具质感的毛玻璃 (`backdrop-blur`)。文字排版使用大行高 (`1.8`)，字号不小于 `18px`。**玩家自定义行动：取消斜体，字号与叙述一致或更大。** |
| **决策菜单** | 叙事区域底部延展 | 行动选项列表，需要有明显的霍夫(Hover)反馈动效。 |
| **输入框** | 决策菜单下方/替代决策栏 | 自定义行动输入。**采用非斜体设计，字号与叙述文本同步（18px+），具备高饱和度聚焦边框与毛玻璃背景。** |
...
为增强对大模型的服从性控制，引入 `IMPORTANT_HINT` 变量。
- **机制**：在每次发送请求给大模型时，该字符串会永远附加在整个消息块的*最尾部*（即 `userPrompt` 之后）。
- **用途**：可以用于强调当前场景的极端危险约束，或是动态强行插入某种游戏规则，防止大模型遗忘。

```javascript
// prompts/systemPrompt.ts
export const IMPORTANT_HINT = "【重要提示】：当前场景没有氧气，任何剧烈运动都会加速死亡。";
```
